...
func opsRoutes(opsHandler *OpsHandlerPackage.Handler) []RouteSpec {
	if opsHandler == nil {
		return []RouteSpec{
			...
			NewRouteSpec("ops", http.MethodGet, OpsModuleBasePath, "/coin/users/search", true, "精确搜索硬币操作目标用户", nil),
	...
	return []RouteSpec{
		...
		NewRouteSpec("ops", http.MethodGet, OpsModuleBasePath, "/coin/users/search", true, "精确搜索硬币操作目标用户", opsHandler.SearchCoinUser),
