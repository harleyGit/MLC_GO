...
func hgTestGateway(t *testing.T, eval hgGatewayRateEval, trusted ...netip.Prefix) *HGAPIGateway {
	...
	for _, module := range hgGatewayModulePaths {
		modules[module.name] = ConfigPackage.HGAPIGatewayModulePolicy{Capacity: 10, RefillPerSecond: 2}
	}
	gateway, err := newHGAPIGateway(eval, ConfigPackage.HGAPIGatewayConfig{Enabled: true, TrustedProxyCIDRs: trusted, Modules: modules})
	if err != nil {
