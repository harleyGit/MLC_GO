...
const (
	...
	// SelectOpsStaleApprovingCoinCorrectionsSQL scans only the timeout index prefix and returns one bounded oldest-first batch. It never uses OFFSET.
	SelectOpsStaleApprovingCoinCorrectionsSQL = "SELECT `correction_id`,`user_id`,`request_id`,`ticket_id`,`work_order_id`,`delta`,`reason`,`applicant_id`,`approver_id`,`status`,COALESCE(`transaction_id`,0),COALESCE(`balance_after`,0),`created_at`,`updated_at` FROM `ops_coin_correction` FORCE INDEX (`idx_status_updated_id`) WHERE `status` = 'approving' AND `updated_at` < ? ORDER BY `updated_at`,`id` LIMIT ?"
