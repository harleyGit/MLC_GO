...
LIMIT 1`
	...
	SelectVideoCommentByCommentIDSQL = `SELECT vc.id, vc.comment_id, vc.submission_id, vc.user_id, COALESCE(NULLIF(u.nickname, ''), NULLIF(u.user_name, ''), vc.user_id), COALESCE(u.avatar_url, ''), vc.content, vc.root_comment_id, vc.parent_comment_id, vc.reply_to_user_id, COALESCE(NULLIF(ru.nickname, ''), NULLIF(ru.user_name, ''), vc.reply_to_user_id, ''), vc.like_count, vc.dislike_count, vc.reply_count, COALESCE(vcr.reaction, 'none'), COALESCE(JSON_UNQUOTE(vc.image_urls), '[]'), vc.created_at FROM video_comments vc INNER JOIN users u ON u.user_id = vc.user_id LEFT JOIN users ru ON ru.user_id = vc.reply_to_user_id LEFT JOIN video_comment_reactions vcr ON vcr.comment_id = vc.comment_id AND vcr.user_id = ? WHERE vc.comment_id = ? AND vc.is_deleted = 0 LIMIT 1`
	SelectVideoCommentByRequestIDSQL = `SELECT vc.id, vc.comment_id, vc.submission_id, vc.user_id, COALESCE(NULLIF(u.nickname, ''), NULLIF(u.user_name, ''), vc.user_id), COALESCE(u.avatar_url, ''), vc.content, vc.root_comment_id, vc.parent_comment_id, vc.reply_to_user_id, COALESCE(NULLIF(ru.nickname, ''), NULLIF(ru.user_name, ''), vc.reply_to_user_id, ''), vc.like_count, vc.dislike_count, vc.reply_count, COALESCE(vcr.reaction, 'none'), COALESCE(JSON_UNQUOTE(vc.image_urls), '[]'), vc.created_at FROM video_comments vc INNER JOIN users u ON u.user_id = vc.user_id LEFT JOIN users ru ON ru.user_id = vc.reply_to_user_id LEFT JOIN video_comment_reactions vcr ON vcr.comment_id = vc.comment_id AND vcr.user_id = ? WHERE vc.user_id = ? AND vc.request_id = ? LIMIT 1`

	// latest/hot 顶级评论查询保留完整复合游标，不使用 OFFSET 深分页。
	ListVideoCommentsLatestFirstSQL    = `SELECT vc.id, vc.comment_id, vc.submission_id, vc.user_id, COALESCE(NULLIF(u.nickname, ''), NULLIF(u.user_name, ''), vc.user_id), COALESCE(u.avatar_url, ''), vc.content, vc.root_comment_id, vc.parent_comment_id, vc.reply_to_user_id, COALESCE(NULLIF(ru.nickname, ''), NULLIF(ru.user_name, ''), vc.reply_to_user_id, ''), vc.like_count, vc.dislike_count, vc.reply_count, COALESCE(vcr.reaction, 'none'), COALESCE(JSON_UNQUOTE(vc.image_urls), '[]'), vc.created_at FROM video_comments vc INNER JOIN users u ON u.user_id = vc.user_id LEFT JOIN users ru ON ru.user_id = vc.reply_to_user_id LEFT JOIN video_comment_reactions vcr ON vcr.comment_id = vc.comment_id AND vcr.user_id = ? WHERE vc.submission_id = ? AND vc.is_deleted = 0 AND vc.root_comment_id IS NULL ORDER BY vc.created_at DESC, vc.id DESC LIMIT ?`
	ListVideoCommentsLatestByCursorSQL = `SELECT vc.id, vc.comment_id, vc.submission_id, vc.user_id, COALESCE(NULLIF(u.nickname, ''), NULLIF(u.user_name, ''), vc.user_id), COALESCE(u.avatar_url, ''), vc.content, vc.root_comment_id, vc.parent_comment_id, vc.reply_to_user_id, COALESCE(NULLIF(ru.nickname, ''), NULLIF(ru.user_name, ''), vc.reply_to_user_id, ''), vc.like_count, vc.dislike_count, vc.reply_count, COALESCE(vcr.reaction, 'none'), COALESCE(JSON_UNQUOTE(vc.image_urls), '[]'), vc.created_at FROM video_comments vc INNER JOIN users u ON u.user_id = vc.user_id LEFT JOIN users ru ON ru.user_id = vc.reply_to_user_id LEFT JOIN video_comment_reactions vcr ON vcr.comment_id = vc.comment_id AND vcr.user_id = ? WHERE vc.submission_id = ? AND vc.is_deleted = 0 AND vc.root_comment_id IS NULL AND (vc.created_at, vc.id) < (?, ?) ORDER BY vc.created_at DESC, vc.id DESC LIMIT ?`
	ListVideoCommentsHotFirstSQL       = `SELECT vc.id, vc.comment_id, vc.submission_id, vc.user_id, COALESCE(NULLIF(u.nickname, ''), NULLIF(u.user_name, ''), vc.user_id), COALESCE(u.avatar_url, ''), vc.content, vc.root_comment_id, vc.parent_comment_id, vc.reply_to_user_id, COALESCE(NULLIF(ru.nickname, ''), NULLIF(ru.user_name, ''), vc.reply_to_user_id, ''), vc.like_count, vc.dislike_count, vc.reply_count, COALESCE(vcr.reaction, 'none'), COALESCE(JSON_UNQUOTE(vc.image_urls), '[]'), vc.created_at FROM video_comments vc INNER JOIN users u ON u.user_id = vc.user_id LEFT JOIN users ru ON ru.user_id = vc.reply_to_user_id LEFT JOIN video_comment_reactions vcr ON vcr.comment_id = vc.comment_id AND vcr.user_id = ? WHERE vc.submission_id = ? AND vc.is_deleted = 0 AND vc.root_comment_id IS NULL ORDER BY vc.like_count DESC, vc.reply_count DESC, vc.created_at DESC, vc.id DESC LIMIT ?`
	ListVideoCommentsHotByCursorSQL    = `SELECT vc.id, vc.comment_id, vc.submission_id, vc.user_id, COALESCE(NULLIF(u.nickname, ''), NULLIF(u.user_name, ''), vc.user_id), COALESCE(u.avatar_url, ''), vc.content, vc.root_comment_id, vc.parent_comment_id, vc.reply_to_user_id, COALESCE(NULLIF(ru.nickname, ''), NULLIF(ru.user_name, ''), vc.reply_to_user_id, ''), vc.like_count, vc.dislike_count, vc.reply_count, COALESCE(vcr.reaction, 'none'), COALESCE(JSON_UNQUOTE(vc.image_urls), '[]'), vc.created_at FROM video_comments vc INNER JOIN users u ON u.user_id = vc.user_id LEFT JOIN users ru ON ru.user_id = vc.reply_to_user_id LEFT JOIN video_comment_reactions vcr ON vcr.comment_id = vc.comment_id AND vcr.user_id = ? WHERE vc.submission_id = ? AND vc.is_deleted = 0 AND vc.root_comment_id IS NULL AND (vc.like_count, vc.reply_count, vc.created_at, vc.id) < (?, ?, ?, ?) ORDER BY vc.like_count DESC, vc.reply_count DESC, vc.created_at DESC, vc.id DESC LIMIT ?`
	...
	ListVideoCommentRepliesFirstSQL       = `SELECT vc.id, vc.comment_id, vc.submission_id, vc.user_id, COALESCE(NULLIF(u.nickname, ''), NULLIF(u.user_name, ''), vc.user_id), COALESCE(u.avatar_url, ''), vc.content, vc.root_comment_id, vc.parent_comment_id, vc.reply_to_user_id, COALESCE(NULLIF(ru.nickname, ''), NULLIF(ru.user_name, ''), vc.reply_to_user_id, ''), vc.like_count, vc.dislike_count, vc.reply_count, COALESCE(vcr.reaction, 'none'), COALESCE(JSON_UNQUOTE(vc.image_urls), '[]'), vc.created_at FROM video_comments vc INNER JOIN users u ON u.user_id = vc.user_id LEFT JOIN users ru ON ru.user_id = vc.reply_to_user_id LEFT JOIN video_comment_reactions vcr ON vcr.comment_id = vc.comment_id AND vcr.user_id = ? WHERE vc.submission_id = ? AND vc.root_comment_id = ? AND vc.is_deleted = 0 ORDER BY vc.created_at ASC, vc.id ASC LIMIT ?`
	ListVideoCommentRepliesByCursorSQL    = `SELECT vc.id, vc.comment_id, vc.submission_id, vc.user_id, COALESCE(NULLIF(u.nickname, ''), NULLIF(u.user_name, ''), vc.user_id), COALESCE(u.avatar_url, ''), vc.content, vc.root_comment_id, vc.parent_comment_id, vc.reply_to_user_id, COALESCE(NULLIF(ru.nickname, ''), NULLIF(ru.user_name, ''), vc.reply_to_user_id, ''), vc.like_count, vc.dislike_count, vc.reply_count, COALESCE(vcr.reaction, 'none'), COALESCE(JSON_UNQUOTE(vc.image_urls), '[]'), vc.created_at FROM video_comments vc INNER JOIN users u ON u.user_id = vc.user_id LEFT JOIN users ru ON ru.user_id = vc.reply_to_user_id LEFT JOIN video_comment_reactions vcr ON vcr.comment_id = vc.comment_id AND vcr.user_id = ? WHERE vc.submission_id = ? AND vc.root_comment_id = ? AND vc.is_deleted = 0 AND (vc.created_at, vc.id) > (?, ?) ORDER BY vc.created_at ASC, vc.id ASC LIMIT ?`
