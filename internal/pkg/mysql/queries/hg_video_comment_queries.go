...
LIMIT 1`
	...
	// 父链使用共享锁：并发回复可同时通过，删除的排他锁会等待这些短事务完成。
	SelectVideoCommentParentForUpdateSQL = `SELECT vc.comment_id, vc.root_comment_id, vc.user_id, COALESCE(NULLIF(u.nickname, ''), NULLIF(u.user_name, ''), vc.user_id) FROM video_comments vc INNER JOIN users u ON u.user_id = vc.user_id WHERE vc.comment_id = ? AND vc.submission_id = ? AND vc.is_deleted = 0 LIMIT 1 FOR SHARE`
	SelectVideoCommentRootForUpdateSQL   = `SELECT comment_id FROM video_comments WHERE comment_id = ? AND submission_id = ? AND root_comment_id IS NULL AND is_deleted = 0 LIMIT 1 FOR SHARE`

	// InsertVideoCommentSQL 在短事务内写入顶级或回复评论；关系字段只接受服务端派生值。
	InsertVideoCommentSQL = `INSERT INTO video_comments (comment_id, submission_id, user_id, request_id, root_comment_id, parent_comment_id, reply_to_user_id, reply_to_user_name, content, image_urls) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, CAST(? AS JSON))`
	...
	// 旧根评论首次写入时把原 reply_count 作为 shard 0 基线；后续回复只竞争 256 个分片之一。
	EnsureVideoCommentReplyShardSQL    = `INSERT INTO video_comment_reply_shards (root_comment_id, shard_id, reply_count) SELECT comment_id, 0, reply_count FROM video_comments WHERE comment_id = ? AND root_comment_id IS NULL AND is_deleted = 0 ON DUPLICATE KEY UPDATE root_comment_id = VALUES(root_comment_id)`
	IncrementVideoCommentReplyShardSQL = `INSERT INTO video_comment_reply_shards (root_comment_id, shard_id, reply_count) VALUES (?, ?, 1) ON DUPLICATE KEY UPDATE reply_count = reply_count + 1`
	DecrementVideoCommentReplyShardSQL = `UPDATE video_comment_reply_shards SET reply_count = IF(reply_count > 0, reply_count - 1, 0) WHERE root_comment_id = ? AND shard_id = ?`
	MarkVideoCommentReplyDirtySQL      = `INSERT INTO video_comment_reply_dirty (root_comment_id, shard_id, revision) VALUES (?, ?, 1) ON DUPLICATE KEY UPDATE revision = revision + 1`

	// 评论投影一次 LEFT JOIN 当前用户关系，避免列表中按评论执行 N+1 反应查询。
	SelectVideoCommentByCommentIDSQL = `SELECT vc.id, vc.comment_id, vc.submission_id, vc.user_id, COALESCE(NULLIF(u.nickname, ''), NULLIF(u.user_name, ''), vc.user_id), COALESCE(u.avatar_url, ''), vc.content, vc.root_comment_id, vc.parent_comment_id, vc.reply_to_user_id, COALESCE(vc.reply_to_user_name, ''), vc.like_count, vc.dislike_count, vc.reply_count, COALESCE(vcr.reaction, 'none'), COALESCE(JSON_UNQUOTE(vc.image_urls), '[]'), vc.created_at FROM video_comments vc INNER JOIN users u ON u.user_id = vc.user_id LEFT JOIN video_comment_reactions vcr ON vcr.comment_id = vc.comment_id AND vcr.user_id = ? WHERE vc.comment_id = ? AND vc.is_deleted = 0 LIMIT 1`
	...
	SelectVideoCommentRootListMetadataSQL = `SELECT vc.submission_id, COALESCE((SELECT SUM(s.reply_count) FROM video_comment_reply_shards s WHERE s.root_comment_id = vc.comment_id), vc.reply_count) FROM video_comments vc WHERE vc.comment_id = ? AND vc.root_comment_id IS NULL AND vc.is_deleted = 0 LIMIT 1`
	...
	// 历史回复空快照按单页去重用户集合批量回源；JSON_TABLE 只展开最多 50 个 ID，并通过 users.user_id 唯一索引点查。
	SelectVideoCommentReplyUserNamesSQL = `SELECT u.user_id, COALESCE(NULLIF(u.nickname, ''), NULLIF(u.user_name, ''), u.user_id) FROM JSON_TABLE(CAST(? AS JSON), '$[*]' COLUMNS (user_id VARCHAR(255) PATH '$')) requested INNER JOIN users u ON u.user_id = requested.user_id`
...
ON DUPLICATE KEY UPDATE like_count = GREATEST(like_count + ?, 0), dislike_count = GREATEST(dislike_count + ?, 0)`
	...
	SelectVideoCommentDeleteTargetForUpdateSQL = `SELECT vc.submission_id, vc.root_comment_id, COALESCE((SELECT SUM(s.reply_count) FROM video_comment_reply_shards s WHERE s.root_comment_id = vc.comment_id), vc.reply_count) FROM video_comments vc WHERE vc.comment_id = ? AND vc.user_id = ? AND vc.is_deleted = 0 LIMIT 1 FOR UPDATE`
	...
	ListVideoCommentReplyDirtySQL    = `SELECT root_comment_id, shard_id, revision FROM video_comment_reply_dirty ORDER BY updated_at ASC, root_comment_id ASC, shard_id ASC LIMIT ?`
	ProjectVideoCommentReplyCountSQL = `UPDATE video_comments vc SET reply_count = (SELECT COALESCE(SUM(s.reply_count), 0) FROM video_comment_reply_shards s WHERE s.root_comment_id = vc.comment_id) WHERE vc.comment_id = ?`
	DeleteVideoCommentReplyDirtySQL  = `DELETE FROM video_comment_reply_dirty WHERE root_comment_id = ? AND shard_id = ? AND revision = ?`
	RequeueVideoCommentReplyDirtySQL = `UPDATE video_comment_reply_dirty SET updated_at = CURRENT_TIMESTAMP(6) WHERE root_comment_id = ? AND shard_id = ? AND revision <> ?`
	...
	// 最老年龄观测均 FORCE migration 22/23 建立的覆盖索引并 LIMIT 1，避免空表/小表统计导致生产计划漂移或 filesort。
	SelectVideoCommentReactionDirtyOldestSQL              = `SELECT updated_at FROM video_comment_reaction_dirty FORCE INDEX (idx_video_comment_reaction_dirty_updated) ORDER BY updated_at ASC, comment_id ASC LIMIT 1`
	...
	SelectPendingVideoCommentImageCleanupOldestSQL        = `SELECT created_at FROM video_comment_images FORCE INDEX (idx_video_comment_images_pending_cleanup) WHERE status = 'pending' AND created_at < ? ORDER BY created_at ASC, id ASC LIMIT 1`
	SelectDeletePendingVideoCommentImageCleanupOldestSQL  = `SELECT delete_after FROM video_comment_images FORCE INDEX (idx_video_comment_images_delete_cleanup) WHERE status = 'delete_pending' AND delete_after <= CURRENT_TIMESTAMP(6) ORDER BY delete_after ASC, id ASC LIMIT 1`
	SelectExpiredVideoCommentImageCleanupOldestSQL        = `SELECT cleanup_lease_until FROM video_comment_images FORCE INDEX (idx_video_comment_images_deleting_lease) WHERE status = 'deleting' AND cleanup_lease_until <= CURRENT_TIMESTAMP(6) ORDER BY cleanup_lease_until ASC, id ASC LIMIT 1`
