...
const (
	// SelectDanmakuVideoTargetSQL 通过唯一 video_id 读取已发布、公开且允许弹幕的视频，避免热表写入前扫描稿件。
...
WHERE vf.video_id = ? AND vs.status = 'published' AND vs.visibility = 'public' AND vs.close_danmaku = 0
LIMIT 1`
