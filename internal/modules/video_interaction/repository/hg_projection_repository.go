...
// 状态表使用 Bucket+UpdatedAt+RowID，分片统计使用 Bucket+UpdatedAt+EntityID+ShardID 处理并列行。
...
// HGProjectionHashRange 是预配置的半开 hash bucket 范围 [Start, End)。
type HGProjectionHashRange struct {
	Start uint16
	End   uint16
}
...
func (r *Repository) ListVideoStates(ctx context.Context, cursor HGProjectionCursor, cutoff time.Time, limit int, hashRange HGProjectionHashRange) ([]HGVideoStateProjection, error) {
	rows, err := r.db.QueryContext(ctx, SQLQueriesPackage.SelectVideoStateProjectionPageSQL, hashRange.Start, hashRange.End, cutoff, cursor.Bucket, cursor.Bucket, cursor.UpdatedAt, cursor.Bucket, cursor.UpdatedAt, cursor.RowID, limit)
	if err != nil {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&item.Cursor.Bucket, &item.Cursor.RowID, &item.UserID, &item.SubmissionID, &item.InteractionType, &item.Active, &item.Quantity, &item.Cursor.UpdatedAt); err != nil {
...
func (r *Repository) ListFollowStates(ctx context.Context, cursor HGProjectionCursor, cutoff time.Time, limit int, hashRange HGProjectionHashRange) ([]HGFollowStateProjection, error) {
	rows, err := r.db.QueryContext(ctx, SQLQueriesPackage.SelectFollowStateProjectionPageSQL, hashRange.Start, hashRange.End, cutoff, cursor.Bucket, cursor.Bucket, cursor.UpdatedAt, cursor.Bucket, cursor.UpdatedAt, cursor.RowID, limit)
	if err != nil {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&item.Cursor.Bucket, &item.Cursor.RowID, &item.FollowerID, &item.FolloweeID, &item.Active, &item.Cursor.UpdatedAt); err != nil {
...
func (r *Repository) ListVideoCounts(ctx context.Context, cursor HGProjectionCursor, cutoff time.Time, limit int, hashRange HGProjectionHashRange) ([]HGVideoCountProjection, error) {
	rows, err := r.db.QueryContext(ctx, SQLQueriesPackage.SelectVideoCountProjectionPageSQL,
		hashRange.Start, hashRange.End, cutoff, cursor.Bucket, cursor.Bucket, cursor.UpdatedAt, cursor.Bucket, cursor.UpdatedAt, cursor.EntityID, cursor.Bucket, cursor.UpdatedAt, cursor.EntityID, cursor.ShardID, limit)
	if err != nil {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&item.Cursor.Bucket, &item.Cursor.UpdatedAt, &item.SubmissionID, &item.Cursor.ShardID, &item.LikeCount, &item.CoinCount, &item.FavoriteCount, &item.ShareCount); err != nil {
...
func (r *Repository) ListFollowCounts(ctx context.Context, cursor HGProjectionCursor, cutoff time.Time, limit int, hashRange HGProjectionHashRange) ([]HGFollowCountProjection, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&item.Cursor.Bucket, &item.Cursor.UpdatedAt, &item.UserID, &item.Cursor.ShardID, &item.FollowerCount); err != nil {
