...
- 涉及会影响列表结果的数据写操作后，按现有 key 规则删除对应列表分页缓存和 total 缓存
...
- 固定 SQL 语句统一写入 `internal/infrastructure/persistence/mysql/queries/hg_sql_queries.go`
