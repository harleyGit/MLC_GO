 ...
 * @LastEditTime: 2026-01-13 11:14:30
...
func (readJSONPractice *ReadJSONFilePractice) ExecutePracticeNone() {
	logHG.DebugInfo("协议 读取JSON文件配置 ReadJSONFilePractice ExecutePracticeNone")
}

// json文件读取
func (readJSONPractice *ReadJSONFilePractice) ReadJSONFilePractice_v1() {
	path := ConfigPath()
	logHG.DebugInfo("文件路径 path: ", path)

	Init(path)
	value := confMap["port"]
	logHG.DebugInfo("端口号 port: ", string(value))
}
...
func LoadConfig(path string) (Configs, *MainConfig) {
	...
	if err != nil {
		logHG.ErrInfo("load config conf failed: ", err)
	}
	mainConfig := &MainConfig{}
	// 用于将 JSON 字节数据 转换为 Go 结构体或 map。
	err = json.Unmarshal(buf, mainConfig)
	if err != nil {
		logHG.ErrInfo("decode config file failed:", string(buf), err)
	}
	// 创建一个 Configs 映射
...
func Init(path string) *MainConfig {
	if confModel != nil && path != configPath {
		logHG.ErrInfo("the config is already initialized, oldPath=", configPath, "path= ", path)
	}
