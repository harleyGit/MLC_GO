...
func (this *myService) Start() error {
	...
	hglog.DebugInfo("Start: 启动服务")

	go func() {
		for {
			select {
			case <- this.quit:
				hglog.DebugInfo("收到 quit 信号， 停止服务 goroutine")
				return

			default:
				hglog.DebugInfo("服务运行中。。。。。")
...
func GO_SVC_my_service() {
	...
	if err := svc.Run(service); err != nil {
		hglog.ErrInfo(" 服务出错❌ ", err)
	}
	hglog.DebugInfo("服务已经退出")
}
